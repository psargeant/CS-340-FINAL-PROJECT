...
app.put('/update-unit', function(req, res) {
    ...
    let updateQuery = `UPDATE Units 
                       SET is_available = ?,
                           ...
                           year = ?
                       WHERE unit_ID = ? AND unit_number = ?`;

    let updateValues = [isAvailable, numBedroom, numBathroom, squareFeet, rentPrice, previousYearIncome, year, unitID, unitNumber];
...
app.put('/update-rental-agreement', function(req, res) {
    ...
    if (isNaN(rentalAgreementId) || isNaN(unitId) || isNaN(totalRentBalance) || isNaN(securityDeposit)) {
        res.status(400).send(`Invalid entry`);
        return;
    }
